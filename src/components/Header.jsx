...
export default function Header() {
  const { pathname } = useLocation(); //destructuring pathname out of useLocation
        ...
        >
          {headerLinkData.map((el) => (
            <Link
              key={el.id}
              to={el.url}
              className={`${el.url === pathname ? "bg-primary text-white" : "text-primary bg-white"} py-2 px-4  hover:text-blue-100 hover:bg-primary md:rounded-lg`}
            >
              {el.title}
            </Link>
          ))}
